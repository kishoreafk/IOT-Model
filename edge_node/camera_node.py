...
class LiveCameraNode:
    ...
    async def _run(self):
        ...
        logger.info(f"[LiveCameraNode] Camera {self.camera_index} opened.")
        ...
        try:
            while self._running:
                ...
                if not ret:
                    logger.warning("[LiveCameraNode] Frame capture failed, retrying…")
                    await asyncio.sleep(0.1)
                    continue

                now = time.monotonic()
                if now - self._last_inference_time < self.inference_interval:
                ...
                await self._process_frame(pil_image, bgr_frame)

        finally:
            cap.release()
            sync_task.cancel()
            cv2.destroyAllWindows()
            logger.info("[LiveCameraNode] Camera released.")
    ...
    async def _process_frame(self, pil_image: Image.Image, display_frame: np.ndarray):
        ...
        elif decision == "Adapt_Local":
            await self._handle_adapt_local(pil_image, top_label)

        elif decision == "Escalate_Hub":
            await self._handle_escalate_hub(pil_image)
